...
export function expandToMovements(
  ...
  exercises.forEach((ex, exIndex) => {
    if (ex.hold_sec != null) {
      ...
      // No hold time given — nothing to repeat into a timed sequence, so
      // keep it as a single reps-mode movement instead of guessing a duration.
      out.push({
        name: ex.name,
        mode: 'reps',
        duration_sec: null,
    ...
    if (exIndex < exercises.length - 1) {
      out.push(makeRest(defaultRestSec));
    }
  });
